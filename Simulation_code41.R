...
rownames(sim22.result.tot)<-c("22cluster_size_50_p_0.3","22cluster_size_50_p_0.1",
                              "22cluster_size_10_p_0.3","22cluster_size_10_p_0.1",
                              "22cluster_size_20_p_0.1")
write.table(sim22.result.tot,"sim_22cluster_result1_100.txt",row.names=T,col.names=T)



########################simulation scenario 4-1,2 positive scoring clusters#########################################################

#generate the random graph using the number of nodes in the network
#p:the probability of connecting edges
#n.size-the size of 2 positive scoring cluster,node scores in positive scoring clusters-uniform(0.1,2) or uniform(2,4)
#total 2 positive scoring clusters
#within one positive scoring cluster-low node scores-uniform(0.1,2) and high edge scores-uniform(2,4) 
#within last positive scoring cluster-edge scores-low edge scores-uniform(0.1,2) and high node scores-uniform(2,4) 
#node scores in other nodes in the output random graph-uniform(-4,-0.1)
#edge scores in other edges in the output random graph-uniform(-4,-0.1)
rand.pos.network23<-function(network,p,n.size){
  library(igraph)
  library(BioNet)
  g <- erdos.renyi.game(vcount(network), p)
  V(g)$name<-paste("Var",as.character(1:vcount(network)),sep="")
...
sim.test.pos.cluster23<-function(network,n.size,p,n.sim){
  library(igraph)
  library(BioNet)
  tpr.e<-c()
  fnr.e1<-c()
  ...
  for (i in 1:n.sim){
    network.test<-rand.pos.network23(network=network,n.size=n.size,p=p)
    node.score<-V(network.test)$score
    names(node.score)<-V(network.test)$name
  ...
  return(data.frame(tpr_edge=tpr.e,fnr_edge1=fnr.e1,tpr_node=tpr.n,fnr_node1=fnr.n1))}
#Explore the TPR and FNR for 4 isolated clusters in random graph with different cluster size and connection probability
result23.sim121<-sim.test.pos.cluster23(network,n.size=10,p=0.3,n.sim=100)
result23.sim122<-sim.test.pos.cluster23(network,n.size=10,p=0.1,n.sim=100)

result23.sim21<-sim.test.pos.cluster23(network,n.size=50,p=0.3,n.sim=100)
result23.sim22<-sim.test.pos.cluster23(network,n.size=50,p=0.1,n.sim=100)

result23.sim221<-sim.test.pos.cluster23(network,n.size=20,p=0.3,n.sim=100)
result23.sim222<-sim.test.pos.cluster23(network,n.size=20,p=0.1,n.sim=100)

sim.result23.summary.21<-apply(result23.sim21,2,mean)
sim.result23.summary.22<-apply(result23.sim22,2,mean)
sim.result23.summary.121<-apply(result23.sim121,2,mean)
sim.result23.summary.122<-apply(result23.sim122,2,mean)
sim.result23.summary.221<-apply(result23.sim221,2,mean)
sim.result23.summary.222<-apply(result23.sim222,2,mean)


sim23.result.tot<-rbind(sim.result23.summary.21,sim.result23.summary.22,sim.result23.summary.121,sim.result23.summary.122,
                        sim.result23.summary.221,sim.result23.summary.222)

rownames(sim23.result.tot)<-c("23cluster_size_50_p_0.3","23cluster_size_50_p_0.1",
                              "23cluster_size_10_p_0.3","23cluster_size_10_p_0.1","23cluster_size_20_p_0.3",
                              "23cluster_size_20_p_0.1")
write.table(sim23.result.tot,"sim_23cluster_result1_100.txt",row.names=T,col.names=T)
##############################################################################################################################
...
sim.test.pos3.cluster4<-function(network,n.size,m,power,n.sim){
  ...
  return(data.frame(tpr_edge=tpr.e,fnr_edge1=fnr.e1,fnr_edge2=fnr.e2,tpr_node=tpr.n,fnr_node1=fnr.n1,fnr_node2=fnr.n2))}
#Explore the TPR and FNR for 4 isolated clusters in random graph with different cluster size and connection probability
resultab3.sim11<-sim.test.pos3.cluster4(network=network,n.size=50,m=24,power=3,n.sim=2)
resultab3.sim12<-sim.test.pos3.cluster4(network=network,n.size=50,m=24,power=1,n.sim=2)
...
#generate the Barabasi-Albert scale free random graph using the number of nodes in the network
#p:the probability of connecting edges
#n.size-the size of positive scoring cluster3,node scores in positive scoring clusters-uniform(0.1,2) or uniform(2,4)
#total 2 positive scoring clusters-within first high node scores all positive-uniform(2,4) 
#with low edge scores-uniform(0.1,2)
#within last positive scoring cluster-edge scores-low edge scores-uniform(0.1,2) and high node scores-uniform(2,4) 
#node scores in other nodes in the output random graph-uniform(-4,-0.1)
#edge scores in other edges in the output random graph-uniform(-4,-0.1)
network=network
...
power=3
n.size=10
randba.pos23.network4<-function(network,m,power,n.size){
  library(igraph)
  library(BioNet)
  g <- barabasi.game(vcount(network),m=m,power=power,directed=F)
  V(g)$name<-paste("Var",as.character(1:vcount(network)),sep="")
  ...
  #n.size, the size of the positive scoring cluter-the number of the nodes in the cluster
  #p, the percentage of connecting edges 
  #moderate node score and high edge score
  library(MASS)
  ind.cluster1<-unlist(apply(get.edgelist(g,names=F),1,function(x) return(sum(length(intersect(x,seq(1,n.size,1))))==2)))
  if(sum(ind.cluster1)>=1){
    new.edgelist.cluster1<-get.edgelist(g,names=T)[ind.cluster1==1,]
...
sim.test.pos23.cluster4<-function(network,n.size,m,power,n.sim){
  library(igraph)
  library(BioNet)
  tpr.e<-c()
  fnr.e1<-c()
  ...
  for (i in 1:n.sim){
    network.test<-randba.pos23.network4(network=network,m=m,power=power,n.size=n.size)
    node.score<-V(network.test)$score
    names(node.score)<-V(network.test)$name
    ...
    n.start.cluster2<-floor(vcount(network.test)/2)
    tpr.e<-c(tpr.e,(length(intersect(node.num.e,seq(1,n.size,1))))/(n.size))
    fnr.e1<-c(fnr.e1,(length(intersect(node.num.e,seq((vcount(network.test)-n.size+1),vcount(network.test),1)))/n.size))
  ...
  return(data.frame(tpr_edge=tpr.e,fnr_edge1=fnr.e1,tpr_node=tpr.n,fnr_node1=fnr.n1))}
#Explore the TPR and FNR for 4 isolated clusters in random graph with different cluster size and connection probability
resultab23.sim11<-sim.test.pos23.cluster4(network=network,n.size=50,m=24,power=3,n.sim=100)
resultab23.sim12<-sim.test.pos23.cluster4(network=network,n.size=50,m=24,power=1,n.sim=100)
resultab23.sim21<-sim.test.pos23.cluster4(network=network,n.size=50,m=77,power=3,n.sim=100)
resultab23.sim22<-sim.test.pos23.cluster4(network=network,n.size=50,m=77,power=1,n.sim=100)


resultab23.sim23<-sim.test.pos23.cluster4(network=network,n.size=50,m=77,power=2,n.sim=100)

resultab23.sim213<-sim.test.pos23.cluster4(network=network,n.size=50,m=24,power=2,n.sim=100)

resultab23.sim214<-sim.test.pos23.cluster4(network=network,n.size=50,m=24,power=1.5,n.sim=100)
resultab23.sim211<-sim.test.pos23.cluster4(network=network,n.size=20,m=24,power=3,n.sim=100)
resultab23.sim212<-sim.test.pos23.cluster4(network=network,n.size=20,m=24,power=1,n.sim=100)
resultab23.sim221<-sim.test.pos23.cluster4(network=network,n.size=20,m=77,power=3,n.sim=100)
resultab23.sim222<-sim.test.pos23.cluster4(network=network,n.size=20,m=77,power=1,n.sim=100)


resultab23.sim214<-sim.test.pos23.cluster4(network=network,n.size=20,m=77,power=1.5,n.sim=100)

resultab23.sim224<-sim.test.pos23.cluster4(network=network,n.size=20,m=24,power=1.5,n.sim=100)

sim.resultab23.summary.21<-apply(resultab23.sim21,2,mean)
sim.resultab23.summary.22<-apply(resultab23.sim22,2,mean)
sim.resultab23.summary.12<-apply(resultab23.sim12,2,mean)
sim.resultab23.summary.11<-apply(resultab23.sim11,2,mean)

sim.resultab23.summary.211<-apply(resultab23.sim211,2,mean)
sim.resultab23.summary.212<-apply(resultab23.sim212,2,mean)
sim.resultab23.summary.214<-apply(resultab23.sim214,2,mean)

sim.resultab23.summary.221<-apply(resultab23.sim221,2,mean)
sim.resultab23.summary.222<-apply(resultab23.sim222,2,mean)
