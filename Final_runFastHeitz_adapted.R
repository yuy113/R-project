...
edge.score.inter[edge.degree.name]/(mean(c(cluster.degree.inter[x[2]],negnode.degree.inter[x[1]]))))
module.e<- runFastHeinz.e(network=network, node.scores=node.scores,edge.scores=edge.scores)
V(module.e)$score
plotModule(module.e)

  length(intersect(V(module)$name,V(module.vtest)$name))
  
  length(setdiff(E(module)$name,E(module.vtest)$name))
 vcount(module) 
 vcount(module.vtest)
  ecount(module.vtest)
  ecount(module)
  length(setdiff(E(module.vtest)$name,E(module)$name))
######################################################################################################################
