...
for (i in 1:nsim){
  ...
  node.scores<-node.score(da.igraph=network,pval=pval.sim,fdr=0.2)
  
  cluster.size.sim1<-c(cluster.size.sim1,vcount(runFastHeinz.e(network=network, node.scores=node.scores,edge.scores=edge.scores)))
  cluster.size.sim2<-c(cluster.size.sim2,vcount(runFastHeinz(network=network, scores=node.scores)))
  }
percentage.pos.module.e<-mean(cluster.size.sim1)/vcount(network)
percentage.pos.module.n<-mean(cluster.size.sim2)/vcount(network)

percentage.pos.module.e2<-sum(cluster.size.sim1>0)/length(cluster.size.sim1)
percentage.pos.module.n2<-sum(cluster.size.sim2>0)/length(cluster.size.sim2)
...
p=0.3
n.size=50
sim.test.pos.cluster1<-function(network,n.size,p,n.sim){
  library(igraph)
  library(BioNet)
  tpr.e<-c()
  fnr.e1<-c()
  ...
  fnr.n3<-c()
  for (i in 1:n.sim){
    network.test<-rand.pos.network(network=network,n.size=n.size,p=p)
    ...
    fnr.e2<-c(fnr.e2,(length(intersect(node.num.e,seq((vcount(network.test)-3*n.size+1),(vcount(network.test)-2*n.size),1)))/n.size))
    
    fnr.e3<-c(fnr.e3,(length(intersect(node.num.e,seq((vcount(network.test)-n.size+1),vcount(network.test),1)))/n.size))
    ...
    fnr.n2<-c(fnr.n2,(length(intersect(node.num.n,seq((vcount(network.test)-3*n.size+1),(vcount(network.test)-2*n.size),1)))/n.size))



    
    fnr.n3<-c(fnr.n3,(length(intersect(node.num.n,seq((vcount(network.test)-n.size+1),vcount(network.test),1)))/n.size))
    
  }
  return(data.frame(tpr_edge=tpr.e,fnr_edge1=fnr.e1,fnr_edge2=fnr.e2,fnr_edge3=fnr.e3,tpr_node=tpr.n,fnr_node1=fnr.n1,fnr_node2=fnr.n2,fnr_node3=fnr.n3))}
#Explore the TPR and FNR for 4 isolated clusters in random graph with different cluster size and connection probability
result1.sim21<-sim.test.pos.cluster1(network,n.size=50,p=0.3,n.sim=100)
result1.sim22<-sim.test.pos.cluster1(network,n.size=50,p=0.1,n.sim=100)

result1.sim121<-sim.test.pos.cluster1(network,n.size=10,p=0.3,n.sim=100)
result1.sim122<-sim.test.pos.cluster1(network,n.size=10,p=0.1,n.sim=100)
result1.sim221<-sim.test.pos.cluster1(network,n.size=20,p=0.3,n.sim=100)
result1.sim222<-sim.test.pos.cluster1(network,n.size=20,p=0.1,n.sim=100)
...
write.table(sim.result.tot,"sim_result1_100.txt",row.names=T,col.names=T)
  ...
  #p:the probability of connecting edges
  #n.size-the size of positive scoring cluster3,node scores in positive scoring clusters-uniform(0.1,1) or uniform(2,4)
  #total 4 positive scoring clusters-within first one edge scores all positive-uniform(2,4) with high node score or uniform(0.1,1)
  #within another positive scoring cluster-edge scores all negative-uniform(-3,-0.1)
  #within last positive scoring cluster-edge scores-uniform(-3,3)
  #node scores in other nodes in the output random graph-uniform(-3,3)
  #edge scores in other edges in the output random graph-uniform(-3,3)
...
power=3
n.size=10
randba.pos.network4<-function(network,m,power,n.size){
  library(igraph)
  library(BioNet)
...
sim.test.pos.cluster4<-function(network,n.size,m,power,n.sim){
  library(igraph)
  library(BioNet)
  tpr.e<-c()
  fnr.e1<-c()
  ...
  fnr.n3<-c()
  for (i in 1:n.sim){
    network.test<-randba.pos.network4(network=network,m=m,power=power,n.size=n.size)
    node.score<-V(network.test)$score
    names(node.score)<-V(network.test)$name
  ...
  return(data.frame(tpr_edge=tpr.e,fnr_edge1=fnr.e1,fnr_edge2=fnr.e2,fnr_edge3=fnr.e3,tpr_node=tpr.n,fnr_node1=fnr.n1,fnr_node2=fnr.n2,fnr_node3=fnr.n3))}
#Explore the TPR and FNR for 4 isolated clusters in random graph with different cluster size and connection probability
resultab1.sim11<-sim.test.pos.cluster4(network=network,n.size=50,m=24,power=3,n.sim=100)
resultab1.sim12<-sim.test.pos.cluster4(network=network,n.size=50,m=24,power=1,n.sim=100)
resultab1.sim21<-sim.test.pos.cluster4(network=network,n.size=50,m=77,power=3,n.sim=100)
resultab1.sim22<-sim.test.pos.cluster4(network=network,n.size=50,m=77,power=1,n.sim=100)



resultab1.sim211<-sim.test.pos.cluster4(network=network,n.size=20,m=24,power=3,n.sim=100)
resultab1.sim212<-sim.test.pos.cluster4(network=network,n.size=20,m=24,power=1,n.sim=100)
resultab1.sim221<-sim.test.pos.cluster4(network=network,n.size=20,m=77,power=3,n.sim=100)
resultab1.sim222<-sim.test.pos.cluster4(network=network,n.size=20,m=77,power=1,n.sim=100)




sim.resultab1.summary.21<-apply(resultab1.sim21,2,mean)
sim.resultab1.summary.22<-apply(resultab1.sim22,2,mean)
sim.resultab1.summary.12<-apply(resultab1.sim12,2,mean)
sim.resultab1.summary.11<-apply(resultab1.sim11,2,mean)
